...
class Ledger_m extends CI_Model {
	...
	function search_transaction($keyword64)
	{
	    ...
	    if(isset($post->column_name) && $post->keyword_search != '') {
		    $where_sql = " WHERE `description` LIKE " . $this->db->escape('%' . $post->keyword_search . '%') . " OR `amount_type` LIKE " . $this->db->escape('%' . $post->keyword_search . '%') . " OR `amount` LIKE " . $this->db->escape('%' . $post->keyword_search . '%');
	    }
